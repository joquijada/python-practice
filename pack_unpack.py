...
def my_pack_func(first_arg, **kwargs):
	# kwargs has all the name/value pairs that caller passed, if any in packed into single variable
	print(f'first arg is {first_arg}')
	for k, v in kwargs.items():
		print(f'k is {k} and v is {v}')


# I need to unpack STAT_HEADERS because my_pack_func() expects a variable number of keyworded arguments. If I
